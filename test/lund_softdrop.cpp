    ...
    while (reader.Next()){
        ...
        for (size_t ijet=0; ijet < jet_pt->size(); ++ijet) {
            ...
            if (constituents.size() < 2) {
                ...
                lund_max_kt_events_sd.push_back(numeric_limits<int>::quiet_NaN());
                lund_max_kt_secondary_events_sd.push_back(numeric_limits<int>::quiet_NaN());
                continue;
            }
            ...
            vector<fastjet::contrib::LundDeclustering> declusters = lund.result(groomed_jet);
            ...
            double psi1 = lund_psi_jet[max_kt_index]; //Primary plane psi is the declustering with max kT
            ...
            fastjet::PseudoJet softer = declusters[max_kt_index].softer();
            fastjet::PseudoJet groomed_softer = sd(softer);
            if (!groomed_softer.has_structure()) {
                // cannot softdrop this branch (no structure / no cluster sequence)
                lund_coords_events_secondary_x_sd.push_back({});
                lund_coords_events_secondary_y_sd.push_back({});
                lund_kt_events_secondary_sd.push_back({});
                lund_z_events_secondary_sd.push_back({});
            ...
            auto declusts_secondary = lund.result(groomed_softer); //Secondary plane is the declustering of the softer branch of the first declustering
            ...
            double psi2 = lund_psi_jet[max_kt_index]; //Secondary plane psi is the declustering with max kT
