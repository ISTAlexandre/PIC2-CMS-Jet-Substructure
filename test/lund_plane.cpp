...
const double dp = dphi(phi1, phi2);
return std::sqrt(de*de + dp*dp);
}

inline bool isG(int pid){ return std::abs(pid) == 21; }
inline bool isQ(int pid){ int a = std::abs(pid); return (a >= 1 && a <= 6); }

// example codes: -1 unmatched, 0 gg, 1 qqbar, 2 qg, 3 other
inline int split_channel_from_prongs(int pidA, int pidB){
    if (pidA == 0 || pidB == 0) return -1;
  
    if (isG(pidA) && isG(pidB)) return 0; // g -> gg
    if (isQ(pidA) && isQ(pidB) && pidA == -pidB) return 1; // g -> q qbar
    if ( (isQ(pidA) && isG(pidB)) || (isG(pidA) && isQ(pidB)) ) return 2; // q -> q g (or qbar->qbar g)
  
    return 3;
}  

// return index of best match, or -1
inline int match_gen_parton(double pr_eta, double pr_phi,
                            const std::vector<float>& gen_eta,
                            const std::vector<float>& gen_phi,
                            const std::vector<int>&   gen_pdgId,
                            double dr_max = 0.25){
int best = -1;
double best_dr = 1e9;
for (size_t i=0;i<gen_pdgId.size();++i){
    double dr = deltaR(pr_eta, pr_phi, gen_eta[i], gen_phi[i]);
    if (dr < best_dr){
    best_dr = dr;
    best = (int)i;
    }
}
if (best >= 0 && best_dr < dr_max) return best;
return -1;
}
...
int main(int argc, char** argv) {
    ...
    bool hasGen = (tree->GetBranch("gen_parton_eta") &&
               tree->GetBranch("gen_parton_phi") &&
               tree->GetBranch("gen_parton_pdgId"));

    std::unique_ptr<TTreeReaderValue<std::vector<float>>> gen_parton_eta;
    std::unique_ptr<TTreeReaderValue<std::vector<float>>> gen_parton_phi;
    std::unique_ptr<TTreeReaderValue<std::vector<int>>>   gen_parton_pdgId;

    if (hasGen){
    gen_parton_eta   = std::make_unique<TTreeReaderValue<std::vector<float>>>(reader, "gen_parton_eta");
    gen_parton_phi   = std::make_unique<TTreeReaderValue<std::vector<float>>>(reader, "gen_parton_phi");
    gen_parton_pdgId = std::make_unique<TTreeReaderValue<std::vector<int>>>(reader, "gen_parton_pdgId");
    } else {
    std::cout << "No gen_parton_* branches found (data file?) -> channels will be -1\n";
    }
    ...
    std::vector<int> primary_hardest_split_channel;
    std::vector<int> secondary_hardest_split_channel;

    auto br_primary_ch  = tree->Branch("primary_hardest_split_channel",  &primary_hardest_split_channel);
    auto br_secondary_ch= tree->Branch("secondary_hardest_split_channel",&secondary_hardest_split_channel);
    ...
    fastjet::contrib::SecondaryLund_mMDT secondary(0.1);
    ...
    while (reader.Next()) {
        ...
        primary_hardest_split_channel.clear();
        secondary_hardest_split_channel.clear();
        ...
        for (std::size_t ijet = 0; ijet < jet_pt->size(); ++ijet) {
            ...
            vector<fastjet::PseudoJet> jets = fastjet::sorted_by_pt(cs.inclusive_jets(200.0));
            ...
            */
            ...
            if (jets.empty()) {
                // push empty per-jet vectors / default channels
                primary_hardest_split_channel.push_back(-1);
                secondary_hardest_split_channel.push_back(-1);
                lund_coords_events_x.push_back({});
                lund_coords_events_y.push_back({});
                // ... same for other per-jet containers
                continue;
            }
            ...
            if (hasGen && gen_parton_pdgId && (*gen_parton_pdgId)->size() > 0) {
            const auto& gEta = **gen_parton_eta;
            const auto& gPhi = **gen_parton_phi;
            const auto& gPid = **gen_parton_pdgId;

            // ----- primary hardest split (kt_max_index) -----
            if (kt_max_index >= 0) {
                const auto pA = declusts[kt_max_index].harder();
                const auto pB = declusts[kt_max_index].softer();

                int iA = match_gen_parton(pA.eta(), pA.phi_std(), gEta, gPhi, gPid, 0.25);
                int iB = match_gen_parton(pB.eta(), pB.phi_std(), gEta, gPhi, gPid, 0.25);

                int pidA = (iA >= 0) ? gPid[iA] : 0;
                int pidB = (iB >= 0) ? gPid[iB] : 0;

                ch_primary = split_channel_from_prongs(pidA, pidB);
            }

            // ----- secondary hardest split (kt2_max_index) -----
            if (kt2_max_index >= 0) {
                const auto sA = sec_declusts[kt2_max_index].harder();
                const auto sB = sec_declusts[kt2_max_index].softer();

                int iA = match_gen_parton(sA.eta(), sA.phi_std(), gEta, gPhi, gPid, 0.25);
                int iB = match_gen_parton(sB.eta(), sB.phi_std(), gEta, gPhi, gPid, 0.25);

                int pidA = (iA >= 0) ? gPid[iA] : 0;
                int pidB = (iB >= 0) ? gPid[iB] : 0;

                ch_secondary = split_channel_from_prongs(pidA, pidB);
            }
            }

            // store per jet
            primary_hardest_split_channel.push_back(ch_primary);
            secondary_hardest_split_channel.push_back(ch_secondary);
