...
size = int(os.environ.get('OMPI_COMM_WORLD_SIZE', os.environ.get('SIZE', 1)))
...
for fileName in my_files:
    ...
    labelJets = ("akVs5PFJets", "", "RECO")

    handleGenParticles = Handle("std::vector<reco::GenParticle>")
    labelGenParticles = ("hiGenParticles", "", "HISIGNAL")
    ...
    for i, event in enumerate(events):
        ...
        for jet in jets:
            ...
            for const in jet.getJetConstituents():
                if hasattr(const, "isNonnull") and not const.isNonnull():
                    continue
